...
def seed():
    ...
    try:
        ...
        additional_customers = [
            {
                "identifier": "tiktok_user",
                "identifier_type": "TIKTOK",
                "display_name": "Tiktok Customer",
            },
            {
                "identifier": "insta_user",
                "identifier_type": "INSTAGRAM",
                "display_name": "Instagram Customer",
            },
            {
                "identifier": "shop_user",
                "identifier_type": "SHOP",
                "display_name": "Shop Customer",
            },
            {
                "identifier": "web_user",
                "identifier_type": "WEBSITE",
                "display_name": "Website Customer",
            },
        ]
        customers = [walkin]
        for cust_data in additional_customers:
            cust = (
                db.query(Customer)
                .filter(Customer.identifier == cust_data["identifier"])
                .first()
            )
            if not cust:
        ...
        for c in cats:
            existing = (
                ...
                .filter(
                    InventoryStock.category_id == c.id,
                    InventoryStock.reason == "INITIAL_STOCK",
                )
                .first()
            )
            if not existing:
        ...
        for ref, date, amount, cust_key in sales_data:
            if not db.query(Sale).filter(Sale.reference == ref).first():
                sale = Sale(
                    ...
                    sale_date=date,
                    total_amount=amount,
                    customer_id=customer_objs[cust_key].id,
                    channel="SHOP",
                    staff_id="seed",
                )
                db.add(sale)
        ...
        for ref, date, price in bales_data:
            ...
            if bale:
                for cat in cats:
                    if (
                        not db.query(BaleItem)
                        .filter(
                            BaleItem.bale_id == bale.id, BaleItem.category_id == cat.id
                        )
                        .first()
                    ):
                        item = BaleItem(
                            bale_id=bale.id, category_id=cat.id, quantity=10
                        )
                        db.add(item)
        ...
        bales = (
            ...
            .filter(
                Bale.reference.in_(
                    ["BALE001", "BALE002", "BALE003", "BALE004", "BALE005"]
                )
            )
            .all()
        )
        for bale in bales:
            for cat_id in category_ids:
                ...
                if (
                    not db.query(BaleItem)
                    .filter(BaleItem.bale_id == bale.id, BaleItem.category_id == cat_id)
                    .first()
                ):
