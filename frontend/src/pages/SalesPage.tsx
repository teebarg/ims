...
export default function SalesPage() {
  ...
  const filtered = sales.filter(s => {
    ...
    const matchSearch = name.toLowerCase().includes(search.toLowerCase()) || ident.toLowerCase().includes(search.toLowerCase()) || s.id.toLowerCase().includes(search.toLowerCase());
  ...
  const filteredCustomers = customers.filter(c =>
    c.displayName.toLowerCase().includes(customerSearch.toLowerCase()) || c.identifier.toLowerCase().includes(customerSearch.toLowerCase())
  );
  ...
  const handleCreateInlineCustomer = () => {
    ...
    const c = addCustomer({ displayName: newCustForm.displayName.trim(), identifierType: newCustForm.identifierType, identifier: newCustForm.identifier.trim(), phone: newCustForm.phone.trim() || undefined });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <Button onClick={() => { resetSaleForm(); setSaleOpen(true); }}>
                ...
                {filtered.map(sale => {
                  ...
                  return (
                    <tr key={sale.id} className="border-b last:border-0 hover:bg-muted/20 transition-colors">
                      ...
                      <td className="p-3">
                        <button className="text-left hover:underline" onClick={() => navigate(`/customers/${sale.customerId}`)}>
                      ...
                      <td className="p-3">
                        <Badge variant={sale.status === "paid" ? "default" : sale.status === "partial" ? "secondary" : "destructive"} className="text-xs font-normal">
                          {sale.status}
                        </Badge>
                      </td>
                      <td className="p-3">
                        {sale.status !== "paid" && (
                          <Button size="sm" variant="outline" className="text-xs h-7" onClick={() => { setPaymentSaleId(sale.id); setPaymentOpen(true); }}>
              ...
              <CardContent className="p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge variant={sale.status === "paid" ? "default" : sale.status === "partial" ? "secondary" : "destructive"} className="text-xs">
                      {sale.status}
                    </Badge>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground font-mono">{c?.identifier} · {channelLabels[sale.channel]}</p>
                  </div>
                ...
                {sale.status !== "paid" && (
                  <Button size="sm" variant="outline" className="w-full text-xs" onClick={() => { setPaymentSaleId(sale.id); setPaymentOpen(true); }}>
      ...
      <Dialog open={saleOpen} onOpenChange={(open) => { if (!open) resetSaleForm(); setSaleOpen(open); }}>
        <DialogContent className="sm:max-w-lg">
                ...
                {!creatingCustomer ? (
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input value={newCustForm.displayName} onChange={e => setNewCustForm(p => ({ ...p, displayName: e.target.value }))} placeholder="e.g. Bella W." />
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <Label>Type</Label>
                        <Select value={newCustForm.identifierType} onValueChange={(v: IdentifierType) => setNewCustForm(p => ({ ...p, identifierType: v }))}>
                      ...
                      <div>
                        ...
                        <Input value={newCustForm.identifier} onChange={e => setNewCustForm(p => ({ ...p, identifier: e.target.value }))} placeholder="@handle" />
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" onClick={() => setCreatingCustomer(false)}>Cancel</Button>
                      <Button size="sm" onClick={handleCreateInlineCustomer}>Create & Select</Button>
                    </div>
                  </div>
                )}
              </div>
            ...
            {step === 3 && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input type="number" value={salePaid} onChange={e => setSalePaid(e.target.value)} placeholder="0" max={Number(saleTotal)} />
                </div>
                {Number(saleTotal) > 0 && (
                  <div className={`p-3 rounded-lg text-sm font-medium flex justify-between ${balancePreview <= 0 ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}>
          ...
          <DialogFooter className="flex-row justify-between sm:justify-between">
            <div>
              {step > 1 && <Button variant="ghost" size="sm" onClick={() => setStep(s => s - 1)}><ArrowLeft className="h-3 w-3 mr-1" /> Back</Button>}
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => { resetSaleForm(); setSaleOpen(false); }}>Cancel</Button>
              {step < 3 ? (
                <Button onClick={() => setStep(s => s + 1)} disabled={(step === 1 && !selectedCustomerId) || (step === 2 && (!saleItems || !saleTotal))}>
        ...
        <DialogContent className="sm:max-w-sm">
          ...
          {paymentSale && (
            <div className="space-y-4 py-4">
              <div className="flex justify-between text-sm"><span className="text-muted-foreground">Sale</span><span className="font-medium">{paymentSale.id}</span></div>
              <div className="flex justify-between text-sm"><span className="text-muted-foreground">Total</span><span className="font-medium">${paymentSale.total}</span></div>
              <div className="flex justify-between text-sm"><span className="text-muted-foreground">Paid</span><span className="font-medium">${paymentSale.paid}</span></div>
              <div className="flex justify-between text-sm font-medium"><span className="text-destructive">Balance</span><span className="text-destructive">${paymentSale.total - paymentSale.paid}</span></div>
              <div>
                <Label>Payment Amount ($)</Label>
                <Input type="number" value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} max={paymentSale.total - paymentSale.paid} />
              </div>
            </div>
          )}
          <DialogFooter>
